...
def process_form(request):
    if request.method == 'POST':
        try:
            ...
            option_price = calculate_option_price(asset_price, exercise_price, time_to_expiry, risk_free_interest_rate, volatility, dividend_yield)

            print(f'The call option price is: {option_price}')

            response_data = {'optionPrice': option_price}
...
def calculate_option_price(asset_price, exercise_price, time_to_expiry, risk_free_interest_rate, volatility, dividend_yield):
    ...
    d1 = (math.log(asset_price / exercise_price) + ((risk_free_interest_rate - dividend_yield) + (volatility ** 2) / 2) * time_to_expiry) / (volatility * math.sqrt(time_to_expiry))
    d2 = d1 - volatility * math.sqrt(time_to_expiry)

    # Calculate the call option price
    call_option_price = asset_price * math.exp(-dividend_yield * time_to_expiry) * norm.cdf(d1) - exercise_price * math.exp(-risk_free_interest_rate * time_to_expiry) * norm.cdf(d2)

    # Calculate the put option price (if needed)
    # put_option_price = exercise_price * math.exp(-risk_free_interest_rate * time_to_expiry) * norm.cdf(-d2) - asset_price * math.exp(-dividend_yield * time_to_expiry) * norm.cdf(-d1)
